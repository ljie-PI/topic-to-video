...
2. **TTS = 本地 VoxCPM2 克隆音色（默认）。** 用 ultimate cloning（参考 WAV + 其 transcript）。DashScope CosyVoice 作 fallback（`TTS_BACKEND=dashscope`）。**绝不**使用 `npx hyperframes tts`。
...
| `voice-clone.py` | TTS（默认本地 VoxCPM2 克隆，`TTS_BACKEND=dashscope` 切 CosyVoice） | 本地：GPU + `VOXCPM_REF_WAV`；fallback：`DASHSCOPE_API_KEY` |
...
**Python 包（装到 venv 里）：** 本地推理用 `torch`（CUDA 构建）、`voxcpm`（VoxCPM2 TTS）、`qwen-asr`（Qwen3-ASR + ForcedAligner）、`soundfile`；云端 fallback 用 `dashscope`（CosyVoice TTS + Paraformer ASR），以及所有辅助脚本 import 的依赖。本地模型默认从 HF id 自动下载，也可用 `VOXCPM_MODEL` / `QWEN3_ASR_MODEL` / `QWEN3_ALIGNER_MODEL` 指向本地目录。
