...
3. **ASR = DashScope 非实时识别（默认 `paraformer-v2`）。** HTTP REST 异步任务接口，自动识别 sample rate / format，无需 ffprobe 探测。
   ↳ DO NOT：用 Whisper / `npx hyperframes transcribe`；硬编码 sample\_rate；切回 `paraformer-realtime-v2`（实时模型上限 \~5min 且必须手动传 format/sample\_rate）。
...
8. **Composition 委派给 HyperFrames sub-agent。** 主 agent 只写 `composition-handoff.md`，并把固定 contract / design references 物化到项目工作区的 `references/` 下。
