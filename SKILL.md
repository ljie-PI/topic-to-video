...
一段有解说的视频（3-10 分钟），使用：
- **Web 调研** 在写脚本之前把内容夯实在事实上
- **HyperFrames** 用于下游 HTML composition + 渲染（委派出去）
- **CosyVoice**（通过 Aliyun DashScope）实现克隆音色 TTS —— 默认中文
- **Paraformer**（通过 DashScope）拿词级 ASR 时间戳
- 一份可配置的 style hint，交给下游 HyperFrames 子 agent

**输出：** `{work_dir}/{topic_name}/composition/renders/final.mp4`，可直接发布（由 Phase 8 coding 子 agent 产出）。

## 规则与排错

下面这些规则，每一条都对应基线 agent 踩过的某个具体 bug。**不要"改进"它们而不重新跑一遍 `references/gotchas.md` 里的 gauntlet。**

### 研究与脚本
0. **写之前先调研。** Phase 2 是强制的。每个论断都通过 Gemini Deep Research + `web_search` 落地。
   ↳ 切勿：调研之前写脚本；没有明确条件就跳过 Gemini；用户给了 PDF 还跳过 Phase 2a；用训练数据描述一篇论文。
1. **素材选择在 Phase 4，不是 Phase 5。** 写解说之前先跑 vision-analyze。
   ↳ 切勿：catalog 还没建就写解说；跳过 vision-analyze。

### TTS / ASR
2. **TTS = 通过 DashScope 调用 CosyVoice。** 用克隆音色。**绝不**使用 `npx hyperframes tts`。
3. **ASR = Paraformer-realtime-v2。** 先用 ffprobe 探测 sample_rate。
   ↳ 切勿：用 Whisper / `npx hyperframes transcribe`；硬编码 16000。
   ↳ 修复：把真实的 `sample_rate` 传给 `Recognition()`。症状：`status_code=44`。

### 字体与文本
4. **预先 stage 确定性的 WOFF2 字体。** 用 `scripts/fonts-download.sh` 下载，然后让 Phase 8 brief 指向 `fonts/`。
   ↳ 切勿：让主 agent 去解 composition 的字体 CSS。
5. **字体实现归 HyperFrames。** Phase 8 子 agent 使用本地字体，并按 `hyperframes` skill 中的排版规则处理。

### 素材
6. **每个资源都能追溯到 `material-catalog.json`。** 没有 catalog 引用 → 屏幕上不出现该资源。
   ↳ 切勿：嵌入整段原视频；切片段时不带 `-an`。
   ↳ 修复：片段里漏音 → 用 `ffmpeg -c:v copy -an` 重剪。
7. **素材搜索是可选但推荐的。** 用户说"skip materials"或自带全部视觉时跳过。

### Composition
8. **Composition 委派给 HyperFrames 子 agent。** 主 agent 只写 `composition-brief.md`。
   ↳ 切勿：手写 `composition/index.html`、挑选 GSAP 模式，或在本 session 里修 HyperFrames lint。
...
| 8 | `references/phases/composition-render.md` | HyperFrames handoff + 子 agent 渲染 |
...
| `check-cjk-fonts.py` | 渲染后可选的 CJK 字体健全检查 | — |
