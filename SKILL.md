...
- **Qwen3-TTS**（本地 GPU，Apache-2.0）实现克隆音色 TTS —— 默认中文；可选 VoxCPM2，DashScope CosyVoice 作云端 fallback
...
2. **TTS = 本地 Qwen3-TTS 克隆音色（默认）。** 用参考 WAV + 其 transcript 克隆。可选本地 VoxCPM2（`TTS_BACKEND=voxcpm`，中英混排更稳、48kHz）；DashScope CosyVoice 作 fallback（`TTS_BACKEND=dashscope`）。**绝不**使用 `npx hyperframes tts`。
...
| `voice-clone.py` | TTS（默认本地 Qwen3-TTS 克隆；`TTS_BACKEND=voxcpm` 切 VoxCPM2，`=dashscope` 切 CosyVoice） | 本地：GPU + `VOXCPM_REF_WAV`；fallback：`DASHSCOPE_API_KEY` |
