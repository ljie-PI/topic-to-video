...
`harvest_page/`、`extract_frames/`、`vision_analyze/`（含 `topic-context.txt`）、`material-catalog.json`、`scene-material-suggestions.json`、`voice_clone/`、`transcribe/`、`fonts/`、`composition/`、`narration.txt`、`composition-brief.md`。
...
**Python 包（装到 venv 里）：** `dashscope`（CosyVoice TTS + Paraformer ASR）、`cn2an`（transcript 数字规范化），以及所有辅助脚本 import 的依赖。
