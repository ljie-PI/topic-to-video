...
2. **Scene Layout Inventory**：覆盖每个 scene，满足 R20，并记录每个 scene 的 material condition、`layout_role` 及来源（explicit / inferred + fallback reason / catalog evidence）、所有实际存在的 `visual_role` 及来源、命中的 `references/layout-routing.md` sections / rows、`domain_hint` / `template_hint`（如有）、table / chart shape（如适用）、输出朝向、最终 layout presentation / `cross_aspect_strategy`，以及未采用横屏排列的原因（portrait / vertical 时）；没有 visual role 时记录按 `semantic_intent` / `primary_subject` 直接选 layout 的依据，不得虚构 role；若忽略 `template_hint`、改变 presentation 或将 `data_table` 转 chart，记录原因；inferred role 另记 conflict gate、authoring 预判与 Phase 8.3 实测复核结果。
...
1. 图片 / 视频清晰、关键信息完整，scale factor 符合 `T-MEDIA`；catalog source figure / table / chart 在当前可见窗口内清晰、无 accidental clipping，且未被当作 structured unit 抽取或重画。非 reveal peak frame 还必须在当前帧保留必要结构；reveal / callout 的完整性按第 15 项聚合判断。
2. 元素不越界、不截断。
...
15. `viewport_reveal` / source `detail_callout` 的 start / mid / end 或 proof sequence 必须聚合覆盖 catalog 记录的 `focal_region`、关键区域，以及 source figure / table / chart 的轴线、图例、caption、关键曲线和必要 rows / columns，并避开 avoid-region；单个窗口无 accidental clipping，sequence 不得永久遗漏必要结构。若 reveal 放大了素材但错过重点或必要结构，仍记为失败。
