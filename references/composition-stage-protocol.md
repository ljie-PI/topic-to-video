...
4. **Deterministic geometry gate**：运行 `scripts/measure-composition-layout.py {work_dir}/{topic_name}/composition --viewport <WxH>` 或精确复现其指标。命中 `underfilled_content_area`、`center_clustered_layout`、`oversized_gutter`、`undersized_text`、`tight_text_gap`、`underfilled_container` 任一 finding 时，Phase 8.3 失败。
