...
4. **文本元素分步出现**：场景内有 2 个及以上非素材文本元素（要点、数据 callout、标签等）时，不得同时显示。每个元素应在旁白提及对应内容时才出现，出现时机参考 `transcript.json` 词级时间戳。禁止一次性把全屏内容堆满再开始旁白。
5. **图片素材动态化**：图片素材禁止静态陈列，每张图必须有持续的运动效果。整个视频中同一类动效不得重复超过总图片数量的五分之一，确保视觉多样性。
