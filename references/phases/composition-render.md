...
从这一步开始，composition 由一个使用 `hyperframes` 与 `hyperframes-cli` skills 的 coding 子 agent 拥有。`topic-to-video` 主 agent 只负责准备上游资源、写一份精简的 handoff brief、调用子 agent，并对返回的 MP4 做 sanity check。

HyperFrames 子 agent 拥有以下工作：
...
#### 8.2 — 调用一个 coding 子 agent

优先使用当前客户端 / runtime 原生的 sub-agent 或委派工具。prompt 要简短，且应让子 agent 自己从磁盘读 brief 文件，而不是把整份 brief 拼进 shell 命令里。
...
如果环境里没有原生 sub-agent 工具，仅当 CLI fallback 能把上面那段 prompt 原样传过去、并让 coding 子 agent 自己去读 `composition-brief.md` 时，才可以接受。**不要**写 `-p "$(cat composition-brief.md)"`。

不要在主 agent 的会话里驱动 composition。

#### 8.3 — sanity-check 结果

子 agent 返回后，从主 agent 验证：
...
预期：duration 接近目标解说时长，文件大小 > 1 MB，且包含一条音频流。如果有异常，把症状反馈给 HyperFrames 子 agent。**不要**在主 agent 里手工 patch `composition/index.html`。
