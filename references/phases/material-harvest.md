...
默认遇到单 URL 失败继续处理后续 URL；传 `--no-continue-on-error` 才中断。第一次调用会在 `{work_dir}/chrome_profile` 启动 Chrome，窗口和 viewport 大小为 1680x1680；脚本结束后保留 Chrome 进程，后续调用通过 CDP（`http://localhost:9222`）复用。
