...
`media_first` / `video_first` scene 不是无文本，而是主媒体优先、少量文本辅助；如果文本会压缩或遮挡主媒体，应分时出现、降级、外置，或生成相邻 `media_continuation` scene。
...
| `pros_cons` | 单一主体的正反两面：推荐 / 避免、利 / 弊、正确 / 错误、do / don't | positives、negatives、label |
...
| `definition` | 术语 / 概念 + 释义、展开说明 | term、definition、example / note |
| `qa` | 问答 / FAQ：问题 + 回答 | question、answer、note |
...
| `annotated_media` | 图片 / 截图 / figure 上打点、箭头、标注 + 外置解释（论文 figure 为其一） | marker / pin、label、why it matters |
| `quote` | 用户评价、论文结论、作者观点、产品 tagline | quote、source |
| `section_divider` | 章节分隔 / 转场标题卡（长视频分段） | chapter label、kicker、index |
