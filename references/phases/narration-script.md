...
[
  {
    "scene_index": 3,
    "source_material_ref": null,
    "source_no_match": true,
...
| `source_material_ref` / `source_no_match` | 透传 Phase 5.2 结果，方便 Phase 8 判断是素材外置信息还是纯文字信息图 |
...
2. 旁白解释“系统、模块、层、pipeline、输入输出、组件” → `architecture_diagram`。
3. 旁白解释“关系、依赖、生态、传播、连接、谁影响谁” → `network_graph`。
4. 旁白有多个并列点 → `list`。
5. 旁白包含数字、比例、排名、增长、成本、时长 → `data_block` 或 `metric_strip`。
6. 旁白是关键判断、风险、反常识、结论 → `callout`。
7. 已匹配素材是图表 / table / figure 时，优先生成外置信息区和解释 callout，不覆盖素材。
8. 已匹配素材是视频 clip 时，生成简短标签 / 状态说明，不做过重图示阻挡主体。
9. `no_match` scene 应至少生成 2-4 个 `visual_text_units`，避免 Phase 8 退化成单个大标题卡。
