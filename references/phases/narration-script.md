...
- 排行榜、表格、图表、产品卡、大数字等是屏幕信息形态，优先作为 Phase 5.3 `visual_role` 表达；除非素材展示策略本身改变，不新增 `layout_role`。
...
Phase 5.3 必须在 Phase 5.2 之后执行，因为屏幕文本密度取决于已匹配素材的 `layout_role`、尺寸、比例和语义。`media_first` / `video_first` scene 不是无文本，而是主媒体优先、少量文本辅助；如果文本会压缩或遮挡主媒体，应分时出现、降级、外置，或生成相邻 `media_continuation` scene。
...
- `visual_role` 是单个 `visual_text_units[]` 的信息形态；一个 scene 可包含多个 visual text units，但 `visual_role` 本身不表达依赖关系或组合布局。
- `domain_hint` 只说明内容领域，帮助选择表达方式，不参与素材占比或安全区计算。
- `template_hint` 是弱语义提示，不是模板 ID，也不是最终 layout 指令；当前 workflow 不使用全局模板 catalog。
- Phase 5.3 负责把大表是否拆成多个 `chart` units 规划清楚；Phase 8 只实现已有 `data_table` / `chart` units，不凭空发明 chart。
...
| --- | --- | --- |
| `leaderboard` | 排名、Top N、榜单、周榜、产品榜、repo 榜 | rank、name、metric、why it matters、optional material ref |
| `product_card` | 产品 / repo / paper / 项目身份卡 | name、tagline、category、metric、CTA |
| `big_number` | 单个关键数字、hero stat、冲击性指标 | value、unit、label、context |
| `data_table` | 成本表、硬件表、benchmark 表、pricing 表、结构化多行多列表格 | columns、rows、highlighted cells、summary |
| `chart` | 趋势、分布、数值对比、benchmark、成本 / 速度 / 占比可视化 | chart_type、series、axis labels、highlighted points、summary |
...
**Observed role inventory：**

历史 composition 样本中已出现的 `layout_role` 主要是 `no_match`、`media_first`、`viewport_reveal`、`video_first`、`comparison_pair`、`detail_callout`；已定义但样本少见或未见的包括 `band`、`media_continuation`、`comparison_sequence`。历史样本暴露出的信息形态补充为：GitHub leaderboard → `visual_role = "leaderboard"`；带 README / screenshot preview 时仍通过 `layout_role = "viewport_reveal"` / `media_first` / `comparison_sequence` 表达素材策略。Product Hunt cards → `product_card` + `data_block` / `list` / `callout`；有产品截图时 `layout_role = "media_first"`。vLLM / benchmark / cost table → `data_table`，treatment 由 row / column count、数据类型和 orientation 决定。数值趋势 / benchmark / 成本速度对比 → `chart`，可由 Phase 5.3 从过大的 numeric table 拆出。单指标 hook → `big_number`。
