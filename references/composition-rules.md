...
本文件是 Phase 8 HyperFrames composition 的固定规则集合，包含不随具体项目变化的 upstream contracts、visual quality constraints、QA expectations 与 deliverables。它不是 brief，也不是可摘要模板。

Phase 8 主 agent 必须在调用 HyperFrames sub-agent 前，把本文件物化到项目工作区的 `references/composition-rules.md`。sub-agent 只依赖工作区本地副本；如果该文件不存在或不可读，必须停止并反馈主 agent，不得凭默认审美继续制作。

## Rule Precedence

1. Safety / policy / copyright constraints.
2. 本 `composition-rules.md` 中的 hard constraints。
...
- 已预置字体：`fonts/`
- 工作区本地 rules 文件：`references/composition-rules.md`
