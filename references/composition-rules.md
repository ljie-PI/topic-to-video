...
| 竖图 / 竖视频 | 默认使用 key-region-aware `viewport_reveal`：先从 catalog `focal_region`、start / mid / end 关键区域说明或 avoid-region 选定 reveal 窗口和长轴 pan 路径，再使用窗口宽 `0.45-0.65 * CW`、高 `0.75-0.92 * CH`。不要为了完整显示素材而缩成窄条。只有非 `ultra-tall` 且需要完整上下文时，才可使用完整适配显示；此时必须 `MH >= 0.78 * CH`（目标 `0.85-0.95 * CH`）且 `MW >= 0.40 * CW`，剩余横向空间必须用信息区填满。 | 默认使用 key-region-aware `viewport_reveal`；窗口宽 `0.82-0.94 * CW`、高 `0.55-0.78 * CH`，沿长轴 pan 覆盖记录的重点区域。只有非 `ultra-tall` 且完整适配显示仍满足 `MW >= 0.70 * CW`（目标 `0.78-0.92 * CW`）和 `MH >= 0.45 * CH` 时，才可完整显示；不要为了看完整图把宽度缩到不可读。 |
...
| 竖向 `1080x1440` | 不要直接套横屏右侧栏。优先上下分区：上方 / 中部放素材主体，下方或顶部放 shrink-to-fit 信息带；也可使用 60/40 或 55/45 的上下 split。竖图 / 竖视频默认走 key-region-aware `viewport_reveal`，不得用左右窄分栏把主素材缩细；必须给字幕安全区留足空间。`process_flow` / `timeline` / `state_machine` 用纵向节点链；`architecture_diagram` / `network_graph` 用模块层级、focus window 或 primary path；`data_table` / `chart` / `leaderboard` 用分页、分时、主项高亮或拆 scene。 |
...
| `media_first` | 清晰大图作为主体，优先按内容区可用宽高共同计算，避免只用固定 max-width 压低高度。若主素材是 tall / ultra-tall，应改为 key-region-aware `viewport_reveal`；完整适配显示必须满足阈值。 | 横图用 media slab + 上下文本区；tall / ultra-tall 用宽度驱动 reveal；信息多时分时、外置、降级或转 `media_continuation`。 | 主媒体不得被固定标题区或信息块不必要压小。 |
| `media_continuation` | 保持相近位置、尺寸、裁切窗口和动效，只刷新解释文本、局部强调或 `focal_region`。 | 同左，尤其保持主素材在相邻 scene 中稳定。 | 避免素材长时间消失后再出现，也避免硬切到完全不同版式。 |
| `viewport_reveal` | 极端比例或完整适配显示不可读的素材进入 reveal viewport，按长轴慢速 pan / scroll；横屏竖图窗口通常宽 `0.45-0.65 * CW`、高 `0.75-0.92 * CH`。 | 同左，但竖屏横图优先横向 reveal，竖屏 tall / ultra-tall 素材优先宽度驱动纵向 reveal（宽 `0.82-0.94 * CW`、高 `0.55-0.78 * CH`）。 | 必须覆盖 start / mid / end 可见区域和关键内容。 |
...
`viewport_reveal` 只适用于 `layout_role = "viewport_reveal"` 或需要局部可视窗口的 `video_first` / `detail_callout`。外层容器可使用适合 scene 的比例并 `overflow: hidden`；内层图片 / 视频必须保持原始宽高比，不得拉伸。短边对齐容器，长边溢出并沿长轴慢速 pan / scroll；必须使用 catalog `focal_region`、start / mid / end 关键区域说明或 avoid-region 来选定 reveal 窗口和 pan 路径。横屏竖图 reveal viewport 通常宽 `0.45-0.65 * CW`、高 `0.75-0.92 * CH`；竖屏 tall / ultra-tall reveal viewport 通常宽 `0.82-0.94 * CW`、高 `0.55-0.78 * CH`。不得出现 letterbox / pillarbox / accidental clipping。

#### R11 — Media dominance and quality

有图片 / 视频素材的 scene，素材应占据内容区主体（通常 >= 50%），禁止缩成角落邮票贴在大段文字旁。`video_first` 和 `media_first` 的主媒体应优先最大化可视区域：横屏输出中的横屏视频 / 清晰横图通常宽度对齐内容区；竖屏 / 竖向输出中的横屏视频 / 横图通常作为上半屏或中上部 media slab，避免缩成小图。主媒体尺寸必须同时按内容区可用宽度和高度计算，不能只用固定 max-width 导致横屏 wide media 高度过低；横屏中作为主证据的 16:9 / wide image 或 video slab 通常应达到内容区高度的 65-85%，低于约 60% 时必须改布局。tall / ultra-tall 素材默认使用 key-region-aware `viewport_reveal`；完整适配显示时，横屏竖图 / 竖视频必须 `MH >= 0.78 * CH` 且 `MW >= 0.40 * CW`，竖屏 / 竖向 tall media 必须 `MW >= 0.70 * CW` 且 `MH >= 0.45 * CH`。不满足时不得继续缩宽度，应改宽度驱动 reveal。Catalog 图片 / 视频的 rendered size 按主要可见边或短边计算，优先保持在原始素材尺寸的 0.8x-1.5x 范围内；超出范围时必须替换素材或调整布局。多素材 scene 两个素材可用 `comparison_pair`，三个及以上优先 `comparison_sequence` / carousel / 拆 scene；如必须并列，每个素材 >= 30% 内容区且仍可读。图片必须清晰、关键信息完整，原图分辨率应覆盖渲染尺寸，不得可见模糊、像素化、JPG artifacts 或裁掉文字、图表轴线、人物面部、UI 主控件等关键信息。
...
确定性几何检查以 `CW` / `CH` 内容区为基准（viewport 扣除 scene padding 与 R15 字幕安全区）。所有非字幕前景内容的 union box 必须真实来自可见文本、素材、图形、卡片或 callout；隐藏元素、透明占位、空容器和仅用于撑尺寸的 wrapper 不计入填充。无素材 / 纯排版 scene 的内容 union 在横屏中通常应覆盖 `CW >= 0.62` 且 `CH >= 0.45`，在竖向 / 竖屏中通常应覆盖 `CW >= 0.58` 且 `CH >= 0.58`；低于阈值且未在 `composition/DESIGN.md` 标记为 deliberate hero / quote / title-card 时，视为 `underfilled_content_area`。若 union 同时小于 `0.55 * CW` 和 `0.45 * CH` 且视觉中心靠近内容区中心（水平与垂直偏移均 < `0.12`），视为 `center_clustered_layout`。横屏左右任一 gutter > `0.28 * CW`、竖屏上下任一 gutter > `0.24 * CH`，且该轴 union coverage 未达阈值时，视为 `oversized_gutter`。主要 card / panel / column（包括无边框 flex / grid 区域）的内部内容 occupancy 横向 < 60%、纵向 < 55% 或实际内容面积 < 35% 时，视为 `underfilled_container`，必须扩大文本 / 图形、重新分布、拆分或合并区域，而不是用空白背景撑版面。相邻主文本块 / 卡片 / 节点的最小正 gap 不应小于 `min(CW, CH) * 0.018`（通常不低于 18px），否则视为 `tight_text_gap`；也不得用过小 gap 换取表面上的填充率。
...
字号下限不是目标字号。无素材、素材稀疏或内容 union 未充分填满内容区时，主信息文字必须优先放大并扩大文本框，而不是停留在通用下限附近：横屏无素材 scene 的主要正文 / callout 通常应 >= 36px，竖向 / 竖屏通常应 >= 38px；承担主叙事锚点的标题 / 数字 / 核心结论应明显大于正文。若 scene 中不存在主素材且最大主信息字号仍接近正文下限，或文本框宽高明显小于内容区可用尺度，视为 `undersized_text`。
...
Phase 8 几何审计必须能产出可复核指标：content area、non-subtitle content union、axis coverage、gutters、visual-center offset、主要文本字号与 gap、container 内部 occupancy。命中 `underfilled_content_area`、`center_clustered_layout`、`oversized_gutter`、`undersized_text`、`tight_text_gap` 或 `underfilled_container` 时，peak-state audit 失败；不得用延后显示、透明占位、空 wrapper 或临时隐藏 peak-state 元素来规避。
