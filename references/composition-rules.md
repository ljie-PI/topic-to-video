...
| R8 | 每 scene 单独 authoring，MUST-NOT 套统一模板；按 `material × layout_role × visual_role × orientation` 解析 `layout-routing.md`；缺失 `layout_role` 按素材 fallback，缺失 `visual_role` 不虚构 | [R8](#r8--scene-specific-layout) |
