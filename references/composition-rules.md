...
本文件是 Phase 8 HyperFrames composition 的固定执行契约，包含不随具体项目变化的 hard contracts、visual quality constraints 与 deliverables。它不是 brief，也不是可摘要模板。

Phase 8 主 agent 必须在调用 HyperFrames sub-agent 前，把本文件物化到项目工作区的 `references/composition-contract.md`。sub-agent 只依赖工作区本地副本；如果该文件不存在或不可读，必须停止并反馈主 agent，不得凭默认审美继续制作。

## Rule Precedence

1. Safety / policy / copyright constraints.
2. 本 `composition-contract.md` 中的 hard constraints。
3. `composition-handoff.md` 中的 user-derived customized rules。
4. 项目 handoff 指定的 design 文件，例如 `references/design-moon.md`。
5. HyperFrames / GSAP 等下游 skill 的默认建议。

Customized rules 可以收紧本契约，但不能静默放宽 hard constraints。若 customized rule 与本契约冲突，主 agent 必须在 handoff 中标注；sub-agent 必须以本契约为底线，并在 `composition/DESIGN.md` 记录冲突处理，不得自行选择更宽松规则。

## Expected Inputs

具体路径由项目的 `composition-handoff.md` 提供。默认工作区结构通常包含：
...
- 已预置字体：`fonts/`
- 工作区本地 contract：`references/composition-contract.md`
- 工作区本地设计规范文件（如适用）：`references/design-<theme>.md`
...
9. **素材容器必须紧贴素材，不允许可见间隙 / 错位**：
   ...
   - **不要在同一素材容器上同时写死 `width` 和 `max-height`/`height`**。CSS 中显式尺寸约束会覆盖 `aspect-ratio`：当 `容器宽 ÷ 素材AR > max-height` 时，高度被钳矮、盒子变得比素材宽，`object-fit: contain` 立刻在左右露出容器底色（即使已声明 aspect-ratio）。按"布局尺寸"定框的布局类最易踩此坑。
...
3. **多个文本元素必须按句子级节奏逐个出现**：场景内 >= 2 个非素材文本元素（标题、要点、数据 callout、标签等）禁止在 scene 开头同时显示。先按 `transcript.json` 中的句号 / 问号 / 感叹号等切出完整句子；某个文本元素服务于哪一句，就在该句开始前短暂提前显示，保证旁白读到这句话时相关文本已经可见。
