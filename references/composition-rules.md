...
确定性几何检查以 `CW` / `CH` 内容区为基准（viewport 扣除 scene padding 与 R15 字幕安全区）。所有非字幕前景内容的 union box 必须真实来自可见文本、素材、图形、卡片或 callout；隐藏元素、透明占位、空容器和仅用于撑尺寸的 wrapper 不计入填充。无素材 / 纯排版 scene：横屏 `union_width / CW >= 0.62` 且 `union_height / CH >= 0.45`；竖向 / 竖屏 `union_width / CW >= 0.58` 且 `union_height / CH >= 0.58`。有 catalog 素材 scene：横屏 `union_width / CW >= 0.50` 且 `union_height / CH >= 0.38`；竖向 / 竖屏 `union_width / CW >= 0.46` 且 `union_height / CH >= 0.42`。低于阈值且未在 `composition/DESIGN.md` 标记为 deliberate hero / quote / title-card 时，视为 `underfilled_content_area`。若 union 同时小于 `0.55 * CW` 和 `0.45 * CH` 且视觉中心靠近内容区中心（水平与垂直偏移均 < `0.12`），视为 `center_clustered_layout`。无素材 scene 的 horizontal gutter 上限为 `0.28 * CW`、vertical gutter 上限为 `0.24 * CH`；有素材 scene 分别为 `0.32 * CW`、`0.28 * CH`。任一轴 gutter 超限且该轴 union coverage 未达阈值时，视为 `oversized_gutter`。主要 card / panel / column（包括无边框 flex / grid 区域）的内部内容 occupancy 横向 < 60%、纵向 < 55% 或实际内容面积 < 35% 时，视为 `underfilled_container`。相邻主文本块 / 卡片 / 节点的最小正 gap 不得小于 `min(CW, CH) * 0.018`（通常不低于 18px），否则视为 `tight_text_gap`。
