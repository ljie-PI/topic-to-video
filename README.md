...
| 子 agent 支持（Phase 8） | 优先使用当前客户端原生的 sub-agent / 委派工具。仅当用一句短 prompt 让 agent 自己读 `composition-brief.md` 时，CLI fallback 才可接受 |
| `hyperframes` + `hyperframes-cli` skills（Phase 8） | 由 composition 子 agent 加载。子 agent 的 HyperFrames CLI 间接依赖 Node.js；主 agent 自身不写 `composition/index.html` |
...
下面这些风格条目是给交付 brief 的风格路由提示，不是 composition 实现规范；最终设计由 HyperFrames 子 agent 决定。
